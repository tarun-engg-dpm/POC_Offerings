...
import redis
# We MUST use the RedisCluster client for a sharded environment
...
# --- LUA SCRIPT: We can only use a SINGLE-KEY script ---
# This script will be called sequentially for each key.
...
def on_test_start(environment, **kwargs):
    ...
    try:
        ...
        if len(ALL_OFFER_IDS) < environment.parsed_options.offers_to_evaluate:
            print("Error: Not enough offers in JSON for offers-to-evaluate count.")
...
class RedisUser(User):
    ...
    def on_start(self):
        ...
        # We only register the single-key script, as it's the only one we can use.
    ...
    def _claim_offers_one_by_one_cluster_safe(self, user_id, batch_to_try):
        """
        This is the ONLY working pattern for non-co-located keys in a cluster.
        It makes sequential, atomic calls for each key.
        """
        granted = []
        ...
        for offer_id in batch_to_try:
            ...
            if offer_type == "GLOBAL_ONLY":
                ...
                # Step 1: Atomically check the user key
                ...
                if user_result == b"SUCCESS":
                    # Step 2: If user check passes, atomically check the global key
                    ...
                    if global_result == b"SUCCESS":
                        ...
                        # Compensating transaction: Revert the user increment
    ...
    def _secure_n_offers(self, user_id, offers_to_evaluate, n_required):
        claimed, attempted = set(), set()
        max_attempts = n_required + 5
        for _ in range(max_attempts):
            if len(claimed) >= n_required: break
            needed = n_required - len(claimed)
            available = list(set(offers_to_evaluate) - attempted)
            if not available: break
            batch = available[:needed]
            attempted.update(batch)
            newly_claimed = self._claim_offers_one_by_one_cluster_safe(user_id, batch)
            if newly_claimed:
    ...
    def secure_n_offers_task(self):
        ...
        name = "orchestrator:sharded_one_by_one"
        start_time = time.time()
        try:
            granted = self._secure_n_offers(user_id, offers_to_evaluate, opts.n_to_secure)
            ...
            events.request.fire(request_type="redis", name=name, response_time=total_time, response_length=0,
                                exception=e)
            # locust -f single_iter_locust.py --offers-to-evaluate 20 --n-to-secure 10 --web-port 8093